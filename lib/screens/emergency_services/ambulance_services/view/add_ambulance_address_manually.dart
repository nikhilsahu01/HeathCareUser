...
import 'emegency_form.dart';
import '../../accident_trauma/emegency_form_toroma.dart';
import '../ambulance_provider/add_address_manually_provider.dart';


class AddAmbulanceAddressScreen extends StatefulWidget {
  final bool? isHome;

  const AddAmbulanceAddressScreen({super.key, this.isHome});

  @override
  State<AddAmbulanceAddressScreen> createState() => _AddAmbulanceAddressScreenState();
}

class _AddAmbulanceAddressScreenState extends State<AddAmbulanceAddressScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(title: 'Enter Address Ambulance'),
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: ['Home', 'Work', 'Others'].map((type) {
                      ...
                      _buildTextField(pro.receiverNameController, 'Receiver Name'),
                      _buildTextField(
                ...
                CustomAppButton(
                  ...
                  onPressed: () {
                    if (pro.error == null) {
                      if (isYourselfBooking == true) {
                        // ✅ If booking for self → skip emergency form
                        // navSlideFromRight(context, const EmergencyFormScreen());
                      } else {
                        if (pro.landmarkController.text.trim().isEmpty ||
                            pro.floorController.text.trim().isEmpty
                        // ||
                            // pro.receiverNameController.text.trim().isEmpty ||
                            // pro.receiverNumberController.text.trim().isEmpty
                        )
                        {
